...
def get_rank_by_id(sid: str = Query(...), db: Session = Depends(get_db)):
...
def get_students_by_pinyin(spy: str = Query(...), db: Session = Depends(get_db)):
...
def get_students_by_name(sname: str = Query(...), db: Session = Depends(get_db)):
    students = StudentService.get_students_by_name(db, sname)
    return Result.success(data=students)

@router.get("/rank/major")
def get_major_ranking(
    sid: str = Query(..., description="学号，用于获取专业信息"),
    sortBy: str = Query("gpa", description="排序字段: gpa 或 avg"),
    order: str = Query("desc", description="排序方式: desc 或 asc"),
